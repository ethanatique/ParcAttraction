...
def add_critique(data):
    if "attraction_nom" not in data or data["attraction_nom"] == "":
        return False
    ...
    requete = """
        INSERT INTO critiques (attraction_nom, auteur, texte, anonyme) 
        VALUES (?, ?, ?, ?)
    """
    req.insert_in_db(requete, (data["attraction_nom"], auteur, data["texte"], data["anonyme"]))
    return True
